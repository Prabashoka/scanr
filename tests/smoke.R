...
stopifnot(is.numeric(wasserstein_statistic(x[1:20], x[80:100])))
stopifnot(is.integer(refine_cusum(x)))

png_file <- tempfile(fileext = ".png")
png(png_file)
plot_change_points(x, fit, true_change_points = 60L)
plot_vote_scree(fit)
plot_window_votes(fit)
plot_thresholds(fit, window_size = 15L)
dev.off()
unlink(png_file)
